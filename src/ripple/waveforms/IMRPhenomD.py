...
def get_inspiral_phase(fM_s: Array, theta: Array, coeffs: Array, lambda_sigma: Array) -> Array:
    """
    ...
    * (
        lambda_sigma[2]
        + lambda_sigma[3] * eta
        + lambda_sigma[4] * (eta ** 2.0)
    )
    + (chiPN - 1.0) ** 2.0
    * (
        lambda_sigma[5]
        + lambda_sigma[6] * eta
        + lambda_sigma[7] * (eta ** 2.0)
    )
    + (chiPN - 1.0) ** 3.0
    * (
        lambda_sigma[8]
...
# @jax.jit
def Phase(f: Array, theta: Array, coeffs: Array, transition_freqs: Array, lambda_sigma: Array) -> Array:
    """
    ...
    phi_Ins = get_inspiral_phase(f * M_s, theta, coeffs, lambda_sigma)
    ...
    phi_Ins_f1, dphi_Ins_f1 = jax.value_and_grad(get_inspiral_phase)(
        f1 * M_s, theta, coeffs, lambda_sigma
    )
...
def _gen_IMRPhenomD(
    ...
    Psi = Phase(f, theta_intrinsic, coeffs, transition_freqs, lambda_sigma)
    Mf_ref = f_ref * M_s
    Psi_ref = Phase(f_ref, theta_intrinsic, coeffs, transition_freqs, lambda_sigma)
...
# @jax.jit
def gen_IMRPhenomD(f: Array, params: Array, f_ref: float, lambda_sigma: Array):
    """
    ...
    h0 = _gen_IMRPhenomD(f, theta_intrinsic, theta_extrinsic, coeffs, f_ref, lambda_sigma)
    return h0


# @jax.jit
def gen_IMRPhenomD_polar(f: Array, params: Array, f_ref: float, lambda_sigma: Array):
    """
    Generate PhenomD frequency domain waveform following 1508.07253.
    vars array contains both intrinsic and extrinsic variables
    theta = [Mchirp, eta, chi1, chi2, D, tc, phic]
    ...
    """
    iota = params[7]
    h0 = gen_IMRPhenomD(f, params, f_ref, lambda_sigma)
